...
export const insertVisitSchema = createInsertSchema(visits, {
  ...
  contactMobile: z.string().length(10, "Mobile number must be exactly 10 digits").regex(/^\d+$/, "Mobile number must be numerical"),
  phoneNumber: z.string().length(10, "Phone number must be exactly 10 digits").regex(/^\d+$/, "Phone number must be numerical"),
  schoolPhone: z.string().length(10, "School phone number must be exactly 10 digits").regex(/^\d+$/, "School phone number must be numerical"),
